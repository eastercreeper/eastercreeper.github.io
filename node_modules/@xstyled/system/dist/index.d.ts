 ...
 */
declare type SynthesizedPath<T extends Record<string, unknown>> = {
    [P in keyof T]: P extends string ? T[P] extends Record<string, unknown> ? `${P}.${SynthesizedPath<T[P]>}` : P : never;
}[keyof T];
...
declare const createStyleGenerator: <TProps extends Props<_xstyled_util.ITheme> = {}>({ getStyle, props, cssGetters, generators, }: {
...
declare const style: <TProps extends Props<_xstyled_util.ITheme> = {}>({ prop, css, themeGet, key, transform, cssProps: cssPropsOption, }: StyleOptions) => StyleGenerator<TProps>;
...
declare type RgbColor = `rgb(${number}, ${number}, ${number})` | `rgba(${number}, ${number}, ${number}, ${number})`;
declare type HslColor = `hsl(${number}, ${number}%, ${number}%)` | `hsla(${number}, ${number}%, ${number}%, ${number})`;
declare type LabColor = `lab(${number}% ${number} ${number})` | `lab(${number}% ${number} ${number} / ${number})`;
declare type HwbColor = `hwb(${number} ${number}% ${number}%)` | `hwb(${number} ${number}% ${number}% / ${number})`;
declare type FnColor = `${string}(${string})`;
declare type ThemeColor<T extends ITheme = Theme> = SynthesizedPath<T['colors']>;
/**
 ...
 * for typos and such.
 */
declare type Color<T extends ITheme = Theme> = ThemeColor<T> | CSS.DataType.NamedColor | CSS.DataType.DeprecatedSystemColor | CSS.Globals | HexColor | RgbColor | HslColor | LabColor | HwbColor | FnColor | 'currentcolor';
...
interface FontVariantProps<T extends ITheme = Theme> {
    fontVariant?: SystemProp<CSS.Property.FontVariant, T>;
}
declare const fontVariant: StyleGenerator<FontVariantProps<Theme>>;
