...
function useViewportWidth() {
  const [width, setWidth] = React__namespace.useState(
...
function createColorStyles(theme, { targetSelector = "body" } = {}) {
  ...
  function getModePropertiesDeclarations(mode) {
    ...
    return toCustomPropertiesDeclarations(
      __spreadValues(__spreadValues({}, colors2), modes2[mode]),
      modeTheme,
      colorKeys,
      XSTYLED_COLORS_PREFIX
    );
...
function useColorModeTheme(theme, mode) {
  ...
  const customPropertiesTheme = React__namespace.useMemo(() => {
    ...
    const colorKeys = getUsedColorKeys(modes);
    return __spreadProps(__spreadValues({}, theme), {
      colors: __spreadProps(__spreadValues(__spreadValues({}, colors), toCustomPropertiesReferences(
        colors,
        theme,
        colorKeys,
        XSTYLED_COLORS_PREFIX
...
const mediaTransform = (rawValue) => {
  ...
  while (matches = QUERY_REGEXP.exec(rawValue)) {
    ...
    if (getter) {
      ...
      values.push(
        (p) => `${open}${prop}${colon}${getter(value)(p)}${close}`
      );
  ...
  while (matches = MATCH_REGEXP.exec(rawValue)) {
    ...
    if (media) {
      ...
      if (getter) {
        values.push(rawValue.slice(lastIndex, matches.index));
        values.push(
          (p) => `${prop}${colon}${getter(value)(p)}${imp || ""}${semi}`
        );
        lastIndex = matches.index + matches[0].length;
      }
    }
